...
function validateMicroPractice(step){
 ...
 for(let i=0;i<rows.length;i++){const p=unitizeRow(rowParts(rows[i]),i===rows.length-1);if(!p.target)continue;const input=document.getElementById('micro-'+i),row=document.querySelector(`[data-micro-row="${i}"]`);if(!input||!String(input.value).trim()){row&&row.classList.add('is-bad');input&&input.focus();toast('أكمل أولاً الفراغ رقم '+(i+1));return false}if(!microValueMatches(input.value,p.target,!!step.case_sensitive)){row&&row.classList.add('is-bad');input.focus();toast('راجع الفراغ رقم '+(i+1));return false}row&&row.classList.remove('is-bad');row&&row.classList.add('is-ok')}
 return true
}
function rowParts(row){const a=Array.isArray(row)?row:[String(row),''];let desc=a[0]||'',eq=String(a[1]||''),t=(a[2]===undefined||a[2]===null)?'':String(a[2]);if(!t&&eq&&eq.indexOf('?')<0&&eq.indexOf('=')>-1){const i=eq.lastIndexOf('='),r=eq.slice(i+1).trim();if(r){t=r;eq=eq.slice(0,i+1)+' ?'}}return{desc:desc,eq:eq,target:t}}
function unitizeRow(p,isLast){if(!isLast||!p.target)return p;const m=/^([\s\S]*\?)\s*([A-Za-z\u00b0%][A-Za-z0-9\u00b7\u00b2\u00b3\/.^-]*)\s*$/.exec(String(p.eq||''));if(!m)return p;const u=m[2];p.eq=m[1];p.desc=String(p.desc||'').replace(/[:\s]+$/,'')+' — اكتب النتيجة مع الوحدة ('+u+')';const vs=[u,u.replace(/\u00b7/g,'.'),u.replace(/\u00b7/g,' '),u.replace(/\u00b7/g,'')],out=[];String(p.target).split('|').forEach(function(a){a=a.trim();if(!a)return;vs.forEach(function(v){v=v.trim();if(!v)return;out.push(a+' '+v);out.push(a+v)})});p.target=out.filter(function(x,i){return out.indexOf(x)===i}).join('|');return p}
function maskAnswer(v){const s=String(v==null?'':v);if(!s||s.indexOf('?')>-1)return s;const i=s.lastIndexOf('=');return (i>-1&&s.slice(i+1).trim())?s.slice(0,i+1)+' ?':s}
function hintHtml(step){const parts=[];if(step.law)parts.push(`<div><b>القانون المستعمل:</b> ${proseHtml(step.law)}</div>`);if(step.simple_explain)parts.push(`<div>💬 ${proseHtml(step.simple_explain)}</div>`);const rows=[...(step.micro||[]),...(step.micro2||[])];if(rows.length){const items=rows.map((r,i)=>{const p=rowParts(r);let eq=p.eq;if(p.target&&i<rows.length-1)eq=eq.replace('?',String(p.target).split('|')[0]);return `<li><b>${i+1}.</b> ${esc(p.desc)}${eq?` <span class="hint-eq">${mathHtml(eq)}</span>`:''}</li>`}).join('');parts.push(`<div><b>خطوات الحل مفصّلة:</b><ol class="hint-steps">${items}</ol><small>الفراغ الأخير اتركه لنفسك ✍️</small></div>`)}if((step.blanks||[]).length){let sub=String(step.prefix||'');(step.blanks||[]).forEach(b=>{sub+='('+b.target+')'+(b.suffix||'')});parts.push(`<div><b>هكذا يكون التعويض:</b> ${mathHtml(sub)}</div>`);if(step.has_root)parts.push(`<div><b>تحت الجذر:</b> ${mathHtml((step.root_prefix||'')+' '+step.root_target+' '+(step.root_suffix||''))}</div>`);parts.push(`<div>احسب بالترتيب: الأقواس أولاً، ثم الضرب والقسمة، واكتب ${esc(step.result_label||'الناتج')} في خانة النتيجة.</div>`)}if(step.hint)parts.push(`<div><b>مفتاح الفكرة:</b> ${proseHtml(step.hint)}</div>`);return parts.join('')}
function choicesHtml(step){if(!(step.choices||[]).length)return '';return `<div class="mcq-box"><b>🧠 اختر التعليل الصحيح:</b>${step.choices.map((c,i)=>`<label class="mcq-opt"><input type="radio" name="mcq" value="${i}"><span>${c}</span></label>`).join('')}</div>`}
function fillInput(el,v){if(!el||v===undefined||v===null)return;el.value=String(v);el.dispatchEvent(new Event('input',{bubbles:true}))}
function revealStep(q,si,step){if((step.choices||[]).length){const r=document.querySelector('input[name="mcq"][value="'+(step.answer_index||0)+'"]');if(r)r.checked=true}(step.micro||[]).forEach((r,i)=>{const p=rowParts(r);if(p.target)fillInput(document.getElementById('micro-'+i),String(p.target).split('|')[0])});if(step.guided_work)fillInput($('#guidedWork'),(step.guided_work.accepted||[''])[0]);if(step.type){if(!step.micro_only)fillInput($('#proof'),step.target)}else{const ins=[...document.querySelectorAll('[data-blank]')];ins.forEach((el,i)=>fillInput(el,(step.blanks[i]||{}).target));if(step.has_root)fillInput($('#rootResult'),step.root_target);fillInput($('#result'),step.result_target)}state.revealed=state.revealed||{};state.revealed[key(q,si)]=true;save();toast('تم إدخال الإجابة الصحيحة في الفراغات — اضغط «تحقق» للتقدم')}
const SYMS=['Δ','ΔP','Δt','θ','π','×','÷','√','²','³','≈','±','→','←','°','m/s','m/s²','kg','N','N·s','kg·m/s','J','−','⌫'];let lastField=null;
function symbolPad(){return `<div class="sym-pad"><b>🔤 مكتبة الرموز</b><div class="sym-row">${SYMS.map(s=>`<button type="button" class="sym-key" data-sym="${attr(s)}">${esc(s)}</button>`).join('')}</div><small>اختر الفراغ أولاً ثم اضغط الرمز</small></div>`}
function wireSymbolPad(){const card=$('#stepCard');if(!card)return;card.querySelectorAll('input').forEach(el=>el.addEventListener('focus',()=>{lastField=el}));card.querySelectorAll('.sym-key').forEach(btn=>btn.addEventListener('click',()=>{const el=(lastField&&card.contains(lastField))?lastField:(card.querySelector('#proof')||card.querySelector('input'));if(!el)return;el.focus();const s=btn.getAttribute('data-sym');let a=el.value.length,b=el.value.length;try{if(el.selectionStart!==null&&el.selectionStart!==undefined){a=el.selectionStart;b=el.selectionEnd}}catch(e){}if(s==='⌫'){const p=(a===b&&a>0)?a-1:a;el.value=el.value.slice(0,p)+el.value.slice(b);try{el.setSelectionRange(p,p)}catch(e){}}else{el.value=el.value.slice(0,a)+s+el.value.slice(b);const c=a+s.length;try{el.setSelectionRange(c,c)}catch(e){}}el.dispatchEvent(new Event('input',{bubbles:true}))}))}
...
function microHtml(step){const rows=[...(step.micro||[]),...(step.micro2||[])];return rows.map((x,idx)=>{const p=rowParts(x);let eq=p.eq;if(p.target)eq=eq.replace('?',String(p.target).split('|')[0]);return `<div class="micro"><b>${idx+1}</b><div><strong>${esc(p.desc)}</strong>${eq?`<div class="micro-eq">${mathHtml(eq)}</div>`:''}</div></div>`}).join('')}
...
function render(){
 ...
 $('#stepCard').innerHTML=`<div class="step-head"><small>الخطوة ${si+1} من ${q.steps.length}</small><span class="swipe-note">↔ اسحب الخطوات أفقيًا</span></div><h2>${esc(step.title)}</h2><div class="law-guide-box"><b>القانون / المعطى</b><div>${proseHtml(step.law||'')}</div></div>${step.simple_explain?`<div class="explain-box">${proseHtml(step.simple_explain)}</div>`:''}${referencesHtml(q,si,step)}${step.type?formulaProof(step,done):formulaInteractive(step,done)}${done?'':symbolPad()}<div class="navrow"><button id="hint" class="btn outline">💡 تلميح مفصّل</button><button id="reveal" class="btn outline" ${done?'disabled':''}>✅ الإجابة الصحيحة</button><button id="check" class="btn primary" ${done?'disabled':''}>تحقق</button></div><div id="hintBox">${state.hints[k]&&!done?`<div class="notice hint-rich">💡 <b>شرح موجّه:</b>${hintHtml(step)}</div>`:''}</div>${done?solution(step):''}<div class="navrow"><button id="prev" class="btn outline" ${si===0?'disabled':''}>السابق</button><button id="next" class="btn blue" ${!done||si===q.steps.length-1?'disabled':''}>التالي</button></div>`;
 $('#hint').onclick=()=>{state.hints[k]=true;save();render()};const _rv=$('#reveal');if(_rv)_rv.onclick=()=>revealStep(q,si,step);$('#check').onclick=()=>checkStep(q,si,step);$('#prev').onclick=()=>moveStep(-1);$('#next').onclick=()=>moveStep(1);wireSymbolPad();wireCardSwipe();metrics();if(pane)showPane(pane)
}
