...
function render(){
 ...
 $('#hint').onclick=()=>{state.hints[k]=true;save();render()};const _rv=$('#reveal');if(_rv)_rv.onclick=()=>revealStep(q,si,step);$('#check').onclick=()=>checkStep(q,si,step);$('#prev').onclick=()=>moveStep(-1);$('#next').onclick=()=>moveStep(1);wireCardSwipe();metrics();if(pane)showPane(pane)
}
